...
function Void(scene, camera, ticker, waves, grid, nodes, roads, sheens) {
  ...
  let buildTopoArray = async () => {
    const mtlres = await fetch((await import("../images/mtl.webp")).default, {
      mode: "cors",
    });
  ...
  let buildHeightLookupTable = () => {
    let gridHeights = mapTable(this.grid.idTable, 1);
    let nodeHeights = mapTable(this.nodes.idTable, 1);
    let roadHeights = mapTable(this.roads.idTable, 1);
    return [
      ...[...Array(this.waves.heights.length).keys()],
      ...gridHeights,
      ...nodeHeights,
      ...roadHeights,
    ];
  ...
  let updateHeightBuffer = (heightBuffer, lookupTable) => {
    for (let i = 0; i < heightBuffer.length; i++) {
      heightBuffer[i] = this.waves.heights[lookupTable[i]];
    }
  };

  let buildSheensBuffer = () => {
    return new Float32Array(new Array(this.sheens.array.length * 4).fill(0));
  };

  let packSheens = (sheenData) => {
    for (let i = 0; i < this.sheens.array.length; i++) {
      sheenData[i * 4] = this.sheens.array[i].origin[0];
      sheenData[i * 4 + 1] = this.sheens.array[i].origin[1];
      sheenData[i * 4 + 2] = this.sheens.array[i].angle[0];
      sheenData[i * 4 + 3] = this.sheens.array[i].angle[1];
    }
  };

  let buildConnectionBuffer = () => {
    let gridConnections = new Array(this.grid.idTable.length).fill(0);
    let nodeConnections = mapConnected(
      this.nodes.idTable,
  ...
  this.initBuffers = () => {
    ...
    let loadDynamicBuffer = () => {
      sheenData = buildSheensBuffer();
      heightData = buildHeightBuffer();

      heightDataLookupTable = buildHeightLookupTable();
  ...
  function mapTable(idTable, times) {
    let destArray = new Array(idTable.length);
    for (let i = 0; i < idTable.length; i++) {
      for (let n = 0; n < times; n++) {
        destArray[i * times + n] = idTable[i] * times + n;
      }
    }
    return destArray;
  }
